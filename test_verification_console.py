...
print(f"  管理员工单任务: {admin_task_no} (id={admin_task_id})")
...
print(f"  巡查员工单任务: {inspector_task_no} (id={inspector_task_id})")
...
print(f"  管理员可见: {len(admin_tasks)} 条")
...
print(f"  巡查员1可见: {len(inspector_tasks)} 条")
...
print("\n--- 11.2 重启后重新验证核心权限 ---")
r = inspector.get(f"{BASE}/verification/tasks")
test("重启后巡查员列表正常", ok(r))
test("重启后巡查员只看到自己的", all(t["operator_id"] == 2 for t in r.json()))

r = inspector.get(f"{BASE}/verification/tasks/{inspector_task_id}")
test("重启后巡查员查看自己详情成功", ok(r))
test("重启后脱敏仍生效", r.json().get("result", {}).get("can_see_batch_detail") == False)

r = inspector.get(f"{BASE}/verification/tasks/{inspector_task_id}/export/json")
test("重启后巡查员导出仍403", r.status_code == 403, f"status={r.status_code}")

r = inspector.get(f"{BASE}/verification/tasks/{admin_task_id}/export/json")
test("重启后巡查员导出别人任务仍403", r.status_code == 403, f"status={r.status_code}")

r = admin.get(f"{BASE}/verification/tasks/{admin_task_id}/export/json")
test("重启后管理员导出仍成功", ok(r), f"status={r.status_code}")

r = admin.get(f"{BASE}/verification/tasks/{inspector_task_id}")
test("重启后管理员看巡查员任务can_see_batch=True",
